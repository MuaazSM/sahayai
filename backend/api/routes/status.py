...
async def check_status(request: StatusRequest, db: AsyncSession = Depends(get_db)):
    """
    Wearable data comes in → runs through the full pipeline.
    Most of the time this short-circuits at Context (normal reading,
    skip reasoning, zero LLM calls). Only burns LLM calls when
    ...
    # Build user message (only if something's wrong)
    user_message = pipeline_state.get("response_text") or None
    if risk_level == "none":
        user_message = None  # don't bother them when everything's fine

    # Build caregiver alert payload
